...
0. **Pick the mode.** Check for `.dev/reviews/<KEY>.md`:
   - If it exists and has findings without a `Resolution:` line → **fix mode**: your work list is the review's Blocking (and, if the user wants, Should-fix) findings, not the plan's tasks. Stay on the existing branch. For each finding: fix it, run the relevant tests, commit with the key (`PROJ-123: fix — <finding summary>`), and append `Resolution: fixed <commit-sha>` under the finding in the review file. If the user decides a finding won't be fixed, record `Resolution: waived — <reason>` instead; never waive silently on your own.
...
2. **Branch.** Create a single branch for the whole ticket, named `<KEY>-<short-kebab-description>` (e.g. `PROJ-123-add-retry-backoff`). One branch per ticket, one commit per task — never a branch per task; task-level granularity lives in the commit history, which GitLab's MR UI can review commit-by-commit. The key in the branch name is what makes GitLab↔Jira linking free, so always include it. First commit on the branch: the `.dev/plans/<KEY>.md` file itself, so the plan travels with the code.

3. **Work task by task, in plan order.** For each task:
   ...
   - Run the relevant Gradle task (`./gradlew test` or the module-scoped task) and confirm green before moving on. A branch that's green after every task is trivial to bisect and review; one that's only green at the end hides which change broke what. If a task is genuinely half of an inseparable pair (interface now, implementation next task) and greening it would mean writing throwaway stubs, it's acceptable to commit red **once**, with `WIP` and the reason in the commit message — but the very next commit must restore green.
   - Commit with the Jira key in the message, e.g. `PROJ-123: add retry backoff to client`. One commit per task — this is the granularity reviewers and `git bisect` will lean on.
   - Tick the task in the plan file (`[ ]` → `[x]`).
...
Don't open the MR or transition the Jira status here. When the branch is green and every task is done, summarize what you built and any deviations from the plan, then point the user at `ticket-review` — and suggest they run it in a fresh conversation so the reviewer isn't anchored by everything above.
